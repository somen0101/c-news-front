  ...
  <v-app>
    <v-app-bar class="header-bar" flat dark dense fixed app clipped-left>
      ...
      <div v-if="checkusername()">
        ようこそ
        <router-link :to="{ name: 'bookmark', params: { username: username } }">{{ username }}</router-link>さん
      </div>
      <div v-else>
        <v-btn text to="/auth">ログイン</v-btn>
        <v-btn text to="/createuser">新規登録</v-btn>
      </div>
    </v-app-bar>
    <v-navigation-drawer v-model="drawer" app clipped floating>
      <v-list shaped dense class="py-5">
        <v-list-item exact router-link :to="{ name:'index' }">
          <v-list-item-icon>
        ...
        <v-list-item exact router-link :to="{ name:'news', params: { domain_tags: 'jp' }}">
          <v-list-item-icon>
