...
export default {
  ...
  watch: {
    '$route' (to) {   
     ...
     if (this.page_path == '/') {  
    axios.get('http://localhost:8000/api/newstopics/?domain_tags=jp&top_news=top').then(
        response => this.info = response.data) 
     this.page_title = '国内トップニュース'    
        } else if(this.page_path == '/news/jp') {
            axios.get('http://localhost:8000/api/newstopics/?domain_tags=jp').then(
        response => this.info = response.data)  
     this.page_title = '国内記事'
        } else if(this.page_path == '/news/us') {
            axios.get('http://localhost:8000/api/newstopics/?domain_tags=us').then(
        response => this.info = response.data)
        this.page_title = 'アメリカ'  
    } else if(this.page_path == '/news/gb') {
            axios.get('http://localhost:8000/api/newstopics/?domain_tags=gb').then(
        response => this.info = response.data)
        this.page_title = 'イギリス'      
    } else if(this.page_path == '/news/kr') {
            axios.get('http://localhost:8000/api/newstopics/?domain_tags=kr').then(
        response => this.info = response.data)
        this.page_title = '韓国'
    } else if(this.page_path == '/news/fr') {
            axios.get('http://localhost:8000/api/newstopics/?domain_tags=fr').then(
        response => this.info = response.data)
        this.page_title = 'フランス'
     }else if(this.page_path == '/news/it') {
            axios.get('http://localhost:8000/api/newstopics/?domain_tags=it').then(
        response => this.info = response.data)
